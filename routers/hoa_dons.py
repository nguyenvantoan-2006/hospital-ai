# routers/hoa_dons.py
# Router quản lý Hóa đơn & Thanh toán viện phí, Quầy Dược (FR-07)
...
# ════════════════════════════════════════════════════════════════════════════════
#  1. DANH SÁCH & TRA CỨU HÓA ĐƠN VIỆN PHÍ
# ════════════════════════════════════════════════════════════════════════════════
...
def get_all_hoa_dons(db: Session = Depends(get_db)):
    ...
    """
    hoa_dons = db.query(models.HoaDon).order_by(models.HoaDon.id.desc()).all()
    results = []
    ...
    for hd in hoa_dons:
        ...
        results.append({
            ...
            "trang_thai_phat_thuoc": getattr(hd, "trang_thai_phat_thuoc", "cho_lay_thuoc"),
            "thoi_gian_phat_thuoc": hd.thoi_gian_phat_thuoc.strftime("%d/%m/%Y %H:%M") if hd.thoi_gian_phat_thuoc else None,
...
def get_phieu_kham_cho_thanh_toan(db: Session = Depends(get_db)):
    ...
    for pk in phieu_khams:
        ...
        if not existing_hd:
            ...
            tien_kham = 100000.0  # Phí khám cố định
...
# ════════════════════════════════════════════════════════════════════════════════
#  2. XÁC NHẬN THANH TOÁN & TRANSACTION TRỪ TỒN KHO AN TOÀN
# ════════════════════════════════════════════════════════════════════════════════
...
def create_or_confirm_hoa_don(data: HoaDonCreateInput, db: Session = Depends(get_db)):
    ...
    Bọc trong Transaction an toàn, tự động trừ kho nếu có kê đơn thuốc.
    """
    try:
        ...
        # 3. Tìm hoặc khởi tạo Hóa Đơn
        hoa_don = db.query(models.HoaDon).filter(models.HoaDon.phieu_kham_id == data.phieu_kham_id).first()
        trang_thai_truoc = hoa_don.trang_thai if hoa_don else "chua_thanh_toan"
        ...
        if hoa_don:
            ...
            hoa_don = models.HoaDon(
                ...
                trang_thai_phat_thuoc="cho_lay_thuoc" if phieu_kham.don_thuocs else "khong_co_thuoc"
            )
            db.add(hoa_don)

        # 4. Nếu vừa chuyển sang 'da_thanh_toan' và trước đó chưa trừ kho
        if data.trang_thai == "da_thanh_toan" and trang_thai_truoc != "da_thanh_toan":
            if phieu_kham.lich_kham:
        ...
        raise HTTPException(
            ...
            detail=f"Lỗi khi xử lý thanh toán hóa đơn: {str(e)}"
        )


# ════════════════════════════════════════════════════════════════════════════════
#  3. QUẦY DƯỢC: CẤP PHÁT THUỐC & TRỪ KHO DƯỢC PHẨM (ACID TRANSACTION)
# ════════════════════════════════════════════════════════════════════════════════
...
def get_don_thuoc_cho_phat(db: Session = Depends(get_db)):
    ...
    for pk in phieu_khams:
        ...
        results.append({
            ...
            "huong_dan_sau_kham": pk.huong_dan_sau_kham or "",
...
def xac_nhan_phat_thuoc(data: schemas.XacNhanPhatThuocInput, db: Session = Depends(get_db)):
    ...
    1. Trừ tồn kho `so_luong_ton` trong bảng `thuocs` an toàn trong Transaction.
    2. Cập nhật hóa đơn sang 'da_thanh_toan' và 'da_lay_thuoc'.
    3. Ghi nhận thời gian phát và tên Dược sĩ.
    """
    try:
        ...
        hoa_don = pk.hoa_don
        # Nếu đã phát thuốc rồi thì không trừ kho lần thứ 2
        if hoa_don and getattr(hoa_don, "trang_thai_phat_thuoc", None) == "da_lay_thuoc":
            return {
                "message": "Đơn thuốc này đã được phát trước đó!",
                "phieu_kham_id": pk.id,
                "hoa_don_id": hoa_don.id,
            ...
            if dt.thuoc:
                ...
                if sl_tru > ton_cu:
                    raise HTTPException(
                        ...
                        detail=f"Thuốc '{dt.thuoc.ten_thuoc}' trong kho chỉ còn {ton_cu}, không đủ số lượng phát ({sl_tru})."
                    )
                ton_moi = ton_cu - sl_tru
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Lỗi khi thực hiện phát thuốc và trừ kho: {str(e)}"
        )


# ════════════════════════════════════════════════════════════════════════════════
#  4. API LẤY CHI TIẾT IN ĐƠN THUỐC & BIÊN LAI (PRINT CONTRACT)
# ════════════════════════════════════════════════════════════════════════════════
...
def get_chi_tiet_don_thuoc_in(phieu_kham_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "huong_dan_sau_kham": pk.huong_dan_sau_kham or "Uống thuốc đúng liều, tái khám theo chỉ định của bác sĩ.",
